...
namespace Color
{
	...
	struct BufferElement
	{
		...
		BufferElement(ShaderDataType type, std::string_view name = "", bool normalized = false)
			: Name(name), Type(type), Size(SizeOfShaderDataType(type)), Offset(0), Normalized(0)
		{
		}
	...
	public:
		...
		static Ref<VertexBuffer> New(size_t size); // Dynamic specialized VBO construction
		static Ref<VertexBuffer> New(const float* vertices, size_t size); // Clasic VBO construction
	};
